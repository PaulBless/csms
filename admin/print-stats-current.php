...
session_start();
	...
	function generateRow($conn){
        ...
        while($row = $query->fetch_assoc()){
    		...
    		while($crow = $cquery->fetch_assoc()){
                $current_year= date('Y');
    			$sql = "SELECT * FROM `enquiries` WHERE `dept_id` = '".$crow['did']."' AND `et_id`='$id' AND Year(created_on)='$current_year'";
    ...
    $content .= '
      	...
      	<h4 align="center">Service Request Statistics Report - '.$current_year.'</h4>
      	<table border="1" cellspacing="0" cellpadding="3">  
      ';  
   	$content .= generateRow($conn);  
    $content .= '</table>';  
    $pdf->writeHTML($content);  
    $pdf->Output('Current-Service-Requests-Statistics-Report.pdf', 'I');
